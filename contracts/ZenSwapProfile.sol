...
// File: @zenswap/zenswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/ZenSwapProfile.sol

pragma solidity ^0.6.0;

/** @title ZenSwapProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract ZenSwapProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public zenToken;
    ...
    uint256 public numberZenToReactivate;
    uint256 public numberZenToRegister;
    uint256 public numberZenToUpdate;
    ...
    constructor(
        IBEP20 _zenToken,
        uint256 _numberZenToReactivate,
        uint256 _numberZenToRegister,
        uint256 _numberZenToUpdate
    ) public {
        zenToken = _zenToken;
        numberZenToReactivate = _numberZenToReactivate;
        numberZenToRegister = _numberZenToRegister;
        numberZenToUpdate = _numberZenToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends ZEN to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer ZEN tokens to this contract
        zenToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberZenToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer ZEN token to this address
        zenToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberZenToUpdate
    ...
    /**
     * @dev Claim ZEN to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        zenToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of ZEN to register
     * Callable only by owner admins.
     */
    function updateNumberZen(
        uint256 newNumberZenToReactivate,
        uint256 _newNumberZenToRegister,
        uint256 _newNumberZenToUpdate
    ) external onlyOwner {
        numberZenToReactivate = newNumberZenToReactivate;
        numberZenToRegister = _newNumberZenToRegister;
        numberZenToUpdate = _newNumberZenToUpdate;
    }
