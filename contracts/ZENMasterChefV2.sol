...
// MasterChefV2 is the master of Zen. He can make Zen and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ZEN is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    // Bonus muliplier for early zen makers.
    ...
    // Update the given pool's ZEN allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingZen(uint256 _pid, address _user) external view returns (uint256) {
    ...
    // Safe zen transfer function, just in case if rounding error causes pool to not have enough ZENs.
    function safeZenTransfer(address _to, uint256 _amount) internal {
        uint256 zenBal = zen.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > zenBal) {
            transferSuccess = zen.transfer(_to, zenBal);
        } else {
            transferSuccess = zen.transfer(_to, _amount);
        }
		
        require(transferSuccess, "safeZenTransfer: transfer failed");
    }
    ...
    //ZenSwap has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _zenPerBlock) public onlyOwner {
        massUpdatePools();
        zenPerBlock = _zenPerBlock;
		
		emit UpdateEmissionRate(msg.sender, _zenPerBlock);
    }
}
