...
// added functionnalities by the zenswap's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when ZEN mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 zenReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Withdraw ZEN tokens from STAKING.
